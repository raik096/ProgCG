...
std::vector<renderable> lamp, car, tree, drone, cube;

renderable r_track, r_terrain;
texture track_texture, terrain_texture;
...
int main(int argc, char** argv)
{
	...
	// SETUP ombre lampioni
	std::vector<frame_buffer_object> lampDepthFbos;
	...
	basic_shader.bind("uLampCutOff");
	basic_shader.SetFloat("uLampCutOff", glm::cos(glm::radians(5.5f))); // Interno
	basic_shader.bind("uLampOuterCutOff");
	basic_shader.SetFloat("uLampOuterCutOff", glm::cos(glm::radians(50.f))); // Esterno
	...
	basic_shader.bind("uLampsNAmount");
	basic_shader.SetInt("uLampsNAmount", r.cars().size() * 2);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if(SHADOW_MAPPING){
			...
			for (int i = 0; i < r.cars().size(); i++) {
				...
				depth_shader.SetFloat("uPlaneApprox", 0.1);
			...
			//LAMPS SHADOWMAP 
			for (int i = 0; i < r.lamps().size(); i++) {
				glBindFramebuffer(GL_FRAMEBUFFER, lampDepthFbos[i].id_fbo);
				glViewport(0, 0, 512, 512);
				glUseProgram(depth_shader.program);

				depth_shader.SetMatrix4x4("uLightMatrix", lampProjectors[i].light_matrix);
				depth_shader.SetFloat("uPlaneApprox", 0.1);
		...
		//Rendering Pass --------------------------------------------------------------------------------------
		{
			...
			{
				basic_shader.bind("uLampLightMatrix[" + std::to_string(i) + "]");
				basic_shader.SetMatrix4x4("uLampLightMatrix[" + std::to_string(i) + "]", lampProjectors[i].light_matrix);

				basic_shader.bind("uLampShadowMap[" + std::to_string(i) + "]");
				BindTextureId(basic_shader, "uLampShadowMap[" + std::to_string(i) + "]", lampDepthFbos[i].id_tex, 10 + i);
			}
...
*/
			...
			for (unsigned int k = 0; k < r.cars().size(); ++k) {
				...
				glm::vec3 headlightPos1 = glm::vec3(glm::translate(stack.m(), glm::vec3(0.7f, .8f, -4.5f)) * glm::vec4(0, 0, 0, 1)); // Cambiata Y e Z
				...
				glm::vec3 headlightPos2 = glm::vec3(glm::translate(stack.m(), glm::vec3(-0.7f, .8f, -4.5f)) * glm::vec4(0, 0, 0, 1)); // Cambiata Y e Z
